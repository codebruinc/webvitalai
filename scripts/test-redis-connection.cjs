/**
 * Test script for Redis connection (CommonJS version)
...
async function testConnections() {
  const configs = [
    ...
    },
    {
      ...
      config: {
        url: `rediss://${process.env.REDIS_USERNAME || 'default'}:${process.env.REDIS_PASSWORD}@${process.env.REDIS_URL || 'redis-19373.c277.us-east-1-3.ec2.redns.redis-cloud.com'}:${process.env.REDIS_PORT || '19373'}`
      }
    },
    {
      name: 'Non-TLS connection (fallback)',
      config: {
        host: process.env.REDIS_URL || 'redis-19373.c277.us-east-1-3.ec2.redns.redis-cloud.com',
        port: parseInt(process.env.REDIS_PORT || '19373'),
        username: process.env.REDIS_USERNAME || 'default',
